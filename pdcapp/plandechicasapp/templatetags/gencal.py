...
class ListCalendar(HTMLCalendar):
    ...
    def formatday(self, day, weekday):
        ...
        :type weekday: int.
        """
        items = self.month_dict
        if not items[day]:
            today_item = False
        else:
            today_item = items[day]
        ...
        v = '<td class="calendar-cell">%d' % (day.day)
        v += '<ul>'
        i=0
        for item in today_item:
            v += '<li class="calendar-event">%s</li>' % (today_item[i])
            v += '\n'
            i+=1
        v+='</ul></td>'
        #return '<td>%d<p>%s</p></td>' % (day.day, today_item[0])
        return v
